# Dama War — App Store Connect Metadata

## Temel Bilgiler

| Alan | Değer |
|------|-------|
| **App Name** | Dama War |
| **Subtitle** | Turkish Checkers Online |
| **Bundle ID** | com.ogzgul.dama |
| **Category** | Games → Board |
| **Secondary Category** | Games → Strategy |
| **Price** | Free |
| **Age Rating** | 4+ |

---

## Description (English — 4000 karakter max)

```
Challenge your friends or test your skills against AI in this authentic Turkish Dama (Checkers) game!

GAME MODES
• Two Players — Play locally on the same device
• vs Computer (Easy) — Relaxed practice for beginners
• vs Computer (Hard) — Challenging computer opponent
• Online Multiplayer — Play with anyone, anywhere in real time

HOW TO PLAY ONLINE
Create a room and share the 6-character code with your friend. They enter the code to connect — no account needed, no registration, just play!

TURKISH DAMA RULES
Turkish Dama is played on an 8×8 board with unique rules:
• Pieces move forward and sideways (3 directions)
• Captures are possible in all 4 directions — including backward
• Chain captures are mandatory — always take the longest capture chain
• Promote a piece to King (♛) by reaching the last row
• Kings move any distance along rows and columns

FEATURES
• Beautiful dark theme with gold accents
• Smooth animations and satisfying piece movements
• Real-time online multiplayer via P2P connection
• No account or login required
• No ads, no in-app purchases — completely free
• Works offline for local and AI modes

Master the ancient Turkish board game and become a Dama champion!
```

---

## Description (Turkish)

```
Arkadaşlarınla oyna ya da yapay zekaya meydan oku — otantik Türk Dama deneyimi!

OYUN MODLARI
• İki Kişi — Aynı cihazda yerel oyun
• Bilgisayara Karşı (Kolay) — Yeni başlayanlar için rahat pratik
• Bilgisayara Karşı (Zor) — Zorlu bilgisayar rakip
• Online Çok Oyunculu — Gerçek zamanlı, her yerden oyna

ONLINE NASIL OYNANIR
Oda oluştur ve 6 haneli kodu arkadaşına gönder. Arkadaşın kodu girerek bağlanır — hesap gerekmiyor, kayıt yok, sadece oyna!

TÜRK DAMA KURALLARI
Türk Dama, 8×8 tahtada eşsiz kurallarla oynanır:
• Taşlar ileri ve yana (3 yön) hareket eder
• Yeme 4 yönde de mümkün — geriye doğru da
• Çok yemeli hamleler zorunludur — en uzun zinciri seç
• Son sıraya ulaşan taş Şah (♛) olur
• Şahlar satranç kalesi gibi istediği kadar ilerler

ÖZELLİKLER
• Altın vurgulu koyu tema
• Akıcı animasyonlar
• P2P bağlantı ile gerçek zamanlı online oyun
• Hesap veya giriş gerekmez
• Reklam yok, uygulama içi satın alma yok — tamamen ücretsiz
• Yerel ve AI modlarda internet gerektirmez
```

---

## Keywords (100 karakter max)

```
dama,checkers,turkish checkers,board game,strategy,multiplayer,online,2 player,classic,turn based
```

---

## Support URL
```
https://GITHUB_KULLANICI_ADIN.github.io/dama-war/support.html
```

## Privacy Policy URL
```
https://GITHUB_KULLANICI_ADIN.github.io/dama-war/
```

---

## Promotional Text (170 karakter — App Store'da değiştirilebilir, review gerektirmez)

```
Play Turkish Dama online with friends! Create a room, share the code, and start playing instantly — no account needed.
```

---

## App Store Screenshot Boyutları (Zorunlu)

| Cihaz | Boyut | Gerekli mi? |
|-------|-------|-------------|
| iPhone 6.7" (15 Pro Max / 16 Plus) | 1290 × 2796 | **ZORUNLU** |
| iPhone 6.5" (11 Pro Max / XS Max) | 1242 × 2688 | Opsiyonel |
| iPad Pro 12.9" | 2048 × 2732 | Opsiyonel |

**En az 1, en fazla 10 screenshot yüklenebilir.**

---

## Age Rating Anketi (App Store Connect'te dolduracaksın)

| Soru | Cevap |
|------|-------|
| Cartoon or Fantasy Violence | None |
| Realistic Violence | None |
| Sexual Content | None |
| Profanity | None |
| Alcohol, Tobacco, Drugs | None |
| Gambling | None |
| Horror/Fear | None |
| Mature/Suggestive Themes | None |
| → **Sonuç: 4+** | ✅ |

---

## App Store Connect Checklist

- [ ] Screenshots yüklendi (en az iPhone 6.7")
- [ ] App icon 1024×1024 yüklendi
- [ ] Açıklama dolduruldu (EN veya TR)
- [ ] Keywords girildi
- [ ] Support URL girildi
- [ ] Privacy Policy URL girildi
- [ ] Age Rating tamamlandı (4+)
- [ ] Category: Games → Board seçildi
- [ ] Fiyat: Free seçildi
- [ ] "Submit for Review" tıklandı
